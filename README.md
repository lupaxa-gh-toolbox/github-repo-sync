...
Organisation aliases may be a single directory name or a relative path under
`clone_path` (for example `Lupaxa/TheLupaxaProject`). Repository aliases must
remain a single directory name.
...
GitHub Repository Sync uses a simple flat command-line interface.

Repository synchronisation is the default operation. Use `--validate` or `--plan` to inspect configuration without modifying repositories. Use `--status` to
audit repository cleanliness without changing working trees, branches, or commits (online mode may fetch remote-tracking refs).
...
| `grs --status`   | Check repositories for clean, synchronised state (may fetch tracking refs).   |
...
Check repository cleanliness and upstream synchronisation.
...
Reset clean local clones after a remote history rewrite (`git-reset-history`).
