...
def validate_optional_organisation_alias(
    value: Any,
    location: str,
) -> str | None:
    """
    Validate an optional local organisation directory alias.

    Organisation aliases may be a single directory name or a relative path
    under ``config.clone_path`` (for example ``Lupaxa/GitHubToolbox``), so
    multiple top-level trees can share one configuration file.
    ...
    if "\\" in alias:
        raise ConfigurationError(
            f"{location} must use '/' as the path separator, not '\\\\'."
        )

    if alias.startswith("/") or alias.endswith("/"):
        raise ConfigurationError(
            f"{location} must be a relative path without a leading or trailing '/'."
        )

    segments = alias.split("/")

    if any(segment == "" for segment in segments):
        raise ConfigurationError(f"{location} must not contain empty path segments.")
