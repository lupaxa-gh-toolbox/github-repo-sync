...
from .status import RepositoryStatusResult
from .styles import (
...
def format_repository_status_check(
    status: RepositoryStatusCheckStatus,
) -> Text:
    """Format a repository status check for table output."""

    return Text(
        REPOSITORY_STATUS_CHECK_LABELS[status],
        style=REPOSITORY_STATUS_CHECK_STYLES[status],
    )
...
def create_status_results_table(
    results: Sequence[RepositoryStatusResult],
) -> Table:
    """Create a detailed table of repository status checks."""

    table = create_table(show_lines=True)
    table.add_column("Repository", style=STYLE_REPOSITORY, no_wrap=True)
    table.add_column("Status", justify="center", no_wrap=True)
    table.add_column("Ahead", justify="right", no_wrap=True)
    table.add_column("Behind", justify="right", no_wrap=True)
    table.add_column("Destination", style=STYLE_PATH, overflow="fold", ratio=2)
    table.add_column("Details", style=STYLE_MUTED, overflow="fold", ratio=2)

    for result in results:
        table.add_row(
            f"{result['organisation']}/{result['repository']}",
            format_repository_status_check(result["status"]),
            str(result["ahead"]),
            str(result["behind"]),
            format_repository_path(result["path"]),
            result["message"],
...
def print_status_results_table(
    results: Sequence[RepositoryStatusResult],
) -> None:
    """Print detailed repository status checks."""

    display.console.print(create_status_results_table(results))


def create_status_summary_table(
    results: Sequence[RepositoryStatusResult],
) -> Table:
    """Create a grouped repository status summary table."""

    counts: Counter[RepositoryStatusCheckStatus] = Counter(
        result["status"] for result in results
    )
    table = create_table(show_header=False, expand=False)
    table.add_column("Status", style=STYLE_DETAIL_LABEL, no_wrap=True)
    table.add_column("Count", justify="right", no_wrap=True)
    table.add_row("Total", Text(str(len(results)), style=STYLE_REPOSITORY))
    table.add_row(
        "Clean",
        Text(str(counts["clean"]), style=REPOSITORY_STATUS_CHECK_STYLES["clean"]),
    )
    table.add_row(
        "Not clean",
        Text(
            str(len(results) - counts["clean"]),
            style=STYLE_WARNING,
        ),
    )
    ...
    for status, count in sorted(
        counts.items(),
        key=lambda item: REPOSITORY_STATUS_CHECK_LABELS[item[0]].casefold(),
    ):
        if status == "clean":
            continue
        table.add_row(
            REPOSITORY_STATUS_CHECK_LABELS[status],
            Text(str(count), style=REPOSITORY_STATUS_CHECK_STYLES[status]),
        )

    return table


def print_status_summary_table(
    results: Sequence[RepositoryStatusResult],
) -> None:
    """Print grouped repository status counts."""

    display.console.print(create_status_summary_table(results))
